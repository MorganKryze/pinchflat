...
| Show whether YouTube is answering ([`3a722bc`](https://github.com/MorganKryze/pinchflat/commit/3a722bc))                      | A block is not on or off. On one address inside one minute, indexing a collection worked, the pre-download check failed and the download was refused outright, so a blocked instance looks busy. Every few minutes a sampler counts what already happened - downloads that finished, errors that were stamped, indexing jobs that completed or failed - and writes one row. It never contacts YouTube. The page reads green only when a download got through, red only when both tiers refuse, and grey when nothing was attempted, which on a caught-up library is most of the night. The history is drawn as fixed segments so a stretch nothing was measuring stays visible instead of being absorbed into the ones either side.                                                                                                                                                                                                  |
...
| Stop indexing or downloading by hand ([`16b103f`](https://github.com/MorganKryze/pinchflat/commit/16b103f))                   | Waiting a block out meant stopping the container, which takes the interface and every reading down with it - so the one time you most want to watch is the one time you cannot. Two switches rather than one, because on a refused address indexing kept working while every download was turned away. Oban's pause lives in memory, so a setting records the decision and the pause is put back once the job runner is up: a restart brings the queues back the way you left them. The automatic backoff leaves alone anything a switch is holding.                                                                                                                                                                                                                                                                                                                                                                                 |
| Optionally ask before resuming after a throttle ([`16116f0`](https://github.com/MorganKryze/pinchflat/commit/16116f0))        | A fixed pause ends whether or not the block has cleared, and if it has not, the queues spend another threshold's worth of refusals finding out. Off by default. Turn it on and the resume asks one question first - the same pre-download check the download path runs, on one video that has already downloaded - and holds for another window if the answer is still a refusal. A verdict on that one video, or an empty library, counts as no answer and resumes, since staying stopped over a deleted upload is worse than resuming into a block that will pause again. Each refused answer makes the next wait longer, to a ceiling of four pauses, so thirteen hours of block costs eight questions spread out rather than twenty-six at a steady two an hour: a constant rate is a signature even when the offsets are scattered. Stop both queue switches and it asks nothing at all, since nothing could act on the answer. |
| Optionally let the yt-dlp config reach every command ([`e4fba20`](https://github.com/MorganKryze/pinchflat/commit/e4fba20))   | The per-media, per-source, per-profile config cascade applied to downloads alone. That is the right scope for an option about output paths and the wrong one for a plugin directory or a player-client override, which have to be in place for the indexing and metadata calls too or they are only half configured. Off by default, since upstream chose that scope deliberately.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
Versions are semver, starting at `1.0.0`. Patch for a fix, minor for a feature, major for anything
that changes how an existing instance behaves without being asked - which on a fork whose whole
premise is that defaults match upstream should be close to never. Releases before `1.0.0` used dates
(`2026.9.2`), which had no room for two releases in a day and drifted a few days ahead of reality
...
spacing, forced-retry priority, queue backoff, the ask-before-resuming probe, automatic set-aside
and the NFO options all sit in Settings, and the two queue switches sit on the status page. One setting starts on rather than off: whether each refused probe waits longer than the last. It does nothing until you turn the probe on, and having opted into asking, you get the quiet version of asking. The orphaned-job rescue window lives in `ORPHANED_JOB_RESCUE_MINUTES`, because Oban reads
...
A `timeout/1` on the download worker, which would let the orphan rescue window come down. And episode
