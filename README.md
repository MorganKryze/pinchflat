...
| Add a read-only API ([`2817781`](https://github.com/MorganKryze/pinchflat/commit/2817781))                                  | Anything a script wanted to know had to be scraped out of the HTML, and `/sources` could not be listed at all — it is a LiveView paging ten at a time over a websocket, so enumerating sources meant walking ids until enough 404s came back. `/api/v1` now serves sources and media as JSON, behind the same token the OPML feed uses, built from explicit maps so adding a column cannot quietly publish it.                                                                                                          |
