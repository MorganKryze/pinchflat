...
| Rescue download jobs orphaned by a restart ([`095af1d`](https://github.com/MorganKryze/pinchflat/commit/095af1d))             | Upstream never enabled `Oban.Plugins.Lifeline`. A job sitting in `executing` when the container dies stays there, and the download worker's uniqueness covers `:executing`, so nothing can queue a replacement for that media item. It freezes. Rescue happens after two hours, long enough that a slow download never looks like a dead node.                                                                                                                                                    |
