...
| Change                                                                                                                      | Why                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| --------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tell an IP throttle apart from an age restriction ([`adf908f`](https://github.com/MorganKryze/pinchflat/commit/adf908f))    | `"Sign in to confirm"` was matched as a prefix, and it covers two opposite messages. `…your age` is permanent; `…you're not a bot` is a throttle that clears on its own. Both were abandoned, and abandoning returned `{:ok, :non_retry}`, so Oban recorded the job as a success and no job was left to retry the item — silently. Retries are now bounded and spread over hours, so they outlast a throttle window instead of being spent inside it.                                                                   |
...
| Put forced retries at the front of the queue ([`d28560f`](https://github.com/MorganKryze/pinchflat/commit/d28560f))         | `force_download_pending` inserted jobs at the default priority with today's timestamp, and Oban orders by priority before `scheduled_at` — so a forced retry queued up behind everything already waiting. Someone clicking that button is asking for those items now; waiting is what happens if they do not click it.                                                                                                                                                                                                  |
