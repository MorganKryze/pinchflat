...
A recorded reason whenever a media item is set aside, an error timestamp so the UI stops showing
week-old messages as current, a backoff that pauses all four yt-dlp queues during a block, and a
health endpoint that makes any of this visible without opening the database.
