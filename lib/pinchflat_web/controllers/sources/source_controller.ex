defmodule PinchflatWeb.Sources.SourceController do
  ...
  def force_download_pending(conn, %{"source_id" => id}) do
    wrap_forced_action(
      ...
      # Upstream inserts these at the worker default of 5, which is what this does too
      # unless the priority has been changed. Lowering it matters more than it sounds:
      # Oban orders by priority before scheduled_at, so at 5 a forced retry queues up
      # behind every job already waiting, which on a source that has been indexing for a
      # while is days. Someone clicking this button is usually asking for these items now.
      &DownloadingHelpers.enqueue_pending_download_tasks(&1, priority: Settings.get!(:forced_download_priority))
    )
  end
